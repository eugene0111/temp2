  ...
  if (status === "success") {
    return (
      <div className="bg-darker/50 p-12 rounded-2xl border border-neon-green/30 text-center backdrop-blur-sm">
        ...
        <p className="text-gray-400">Thank you for applying. We will review your work and get back to you.</p>
    ...
    <div className="bg-darker/50 p-8 rounded-2xl border border-white/10 backdrop-blur-sm">
      <div className="mb-8">
        ...
        <select
          ...
          onChange={(e) => setSelectedPosition(positions.find((p) => p._id === e.target.value))}
        >
          {positions.map((pos) => (
            <option key={pos._id} value={pos._id}>{pos.position}</option>
          ))}
        </select>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        {selectedPosition.customQuestions?.map((q, index) => (
          <div key={index} className="space-y-4 border-t border-white/5 pt-6 first:border-none first:pt-0">
            <div className="flex flex-col gap-4">
              ...
              <div className="space-y-2">
                <label className="block text-sm text-gray-300 font-medium">{q.questionText}</label>
                {q.fieldType === "small text" && (
                  <input name={q.questionText} type="text" className="w-full bg-darkest border border-white/10 rounded-lg px-4 py-3 text-white outline-none focus:border-neon-green transition-all" required />
                )}
                {q.fieldType === "large text" && (
                  <textarea name={q.questionText} className="w-full bg-darkest border border-white/10 rounded-lg px-4 py-3 text-white h-32 outline-none focus:border-neon-green transition-all resize-none" required></textarea>
                )}
                {q.fieldType === "image upload" && (
                  <div className="group relative">
                    <input name={q.questionText} type="file" accept="image/*" className="w-full cursor-pointer text-sm text-gray-400 file:mr-4 file:rounded-full file:border-0 file:bg-neon-green/10 file:px-4 file:py-2 file:text-xs file:font-bold file:text-neon-green hover:file:bg-neon-green hover:file:text-darkest transition-all" required />
                  </div>
                )}
              </div>
            </div>
